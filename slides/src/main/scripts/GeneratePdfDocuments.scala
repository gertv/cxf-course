...
val BASEDIR = new File(project("project.basedir"))
...
def createPdf(input: File) {
  ...
  val builder = new ProcessBuilder("prince", input.getCanonicalPath, "-o", pdf.getCanonicalPath)
  builder.redirectErrorStream(true)
  builder.directory(new File(BASEDIR, "src/main/webapp"))
...
try {
  ...
  case e: Exception => warn("An error occured while generating PDF files: " + e.getMessage)
} finally {
  LOG_FILE.flush
  LOG_FILE.close
}
